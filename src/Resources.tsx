...
import { resource, revision, inform, workspace } from "./client";
import { draft, preferences } from "./cache";
import { operations, type Field as FormField } from "../shared/resources";
import { Icon, IconButton, Field, Dialog, Empty, run } from "./ui";
import {
  editableJob,
  jobPatch,
  endpointBody,
  ...
  platformBody,
  mcpBody,
} from "../shared/resource-forms";
const ProfileAvatar = lazy(() => import("./ProfileAvatar"));
const RuntimeSettings = lazy(() => import("./RuntimeSettings"));
const MixtureModels = lazy(() => import("./MixtureModels"));
const MemoryGraph = lazy(() => import("./MemoryGraph"));
const FallbackModels = lazy(() => import("./FallbackModels"));
...
export default function Resources(props: {
  ...
  let previousSurface = "";
  createEffect(() => {
    if (props.name !== previousSurface) {
      previousSurface = props.name;
      setData(undefined);
      setForm(null);
      setDetail(undefined);
    ...
    if (operation === "memoryConfig") {
      ...
      setForm({
        ...
        fields: (config.fields ?? []).map((field: any) => ({
          key: field.key,
          label: `${field.label}${field.kind === "secret" && field.is_set ? " (saved; blank keeps current)" : ""}`,
          type:
            field.kind === "secret"
              ? "password"
              : field.kind === "boolean"
  ...
  async function saveTextFile() {
    ...
    if (
      ...
      return;
    await resource("saveFile", {}, current);
  ...
  return (
    <div class="resource-page">
      ...
      <Show when={props.name === "backup"}>
        <p>
          Backups are created on your Hermes host. Download them to keep a
          separate copy.
        </p>
        <button
          type="button"
          class="primary"
          onClick={() =>
            void run(async () => setDetail(await resource("backup", {}, {})))
          }
        >
          Create backup
        </button>
        <a class="button" href="/api/download?type=backup" download="">
                  ...
                  <div class="resource-actions">
                    <Show when={props.name === "profiles"}>
                      <button
                        type="button"
                        class="primary"
                        onClick={() => void props.newChat(item.name ?? item.id)}
                      >
                        Chat
                      </button>
                    </Show>
                        ...
                        onClick={() =>
                          setForm({
                            ...
                            fields: provider.env_vars.map((field: any) => ({
                              key: field.key,
                              label: `${field.prompt ?? field.key}${field.is_set ? " (saved; blank keeps current)" : ""}`,
                              type: "password",
                            })),
                          })
                        }
