...
hermes.on("idle", (idleTurn: { conversation: string; startedAt: number }) => {
  // A buffered snapshot of the finished turn must not be written after the
  // idle ingest deletes it; the ingest's startedAt guard only covers rows
  // that have already landed.
  turnBuffer.delete(idleTurn.conversation);
...
hermes.on("complete", (key: string, turn: Turn) => {
  if (turn.stats) {
    void convex
      .mutation(anyApi.workspace.ingest, { completedStats: turn })
      .catch(report);
  }
  void syncHistory(key).catch(report);
